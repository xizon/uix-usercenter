 ...
 *
 * @class 		: Uix_UserCenter_Custom_Metaboxes
...
if ( !class_exists( 'Uix_UserCenter_Custom_Metaboxes' ) ) {
	...
	class Uix_UserCenter_Custom_Metaboxes {
		...
		*
		*/
		public static $handle = 'uix-usercenter-cmb';
		...
		public static function get_directory() {
			return UixUserCenter::plug_directory() .'includes/admin/uix-custom-metaboxes/';
		}
		...
		*/
		...
		public static function admin_body_class( $classes ) {
			return "$classes uix-usercenter-cmb__bodyclass";
		}
...
//
		...
		public static function backstage_scripts() {
				 ...
				 ) 
			  {
					...
					wp_localize_script( self::$handle,  'uix_usercenter_custom_metaboxes_lang', array( 
						'ed_lang'                 => get_locale(),
						'ed_url'                  => self::get_directory() .'',
						'upload_dir_url'          => $upload_dir_url,
						'ed_media_title'          => __( 'Select Files', 'uix-usercenter' ),
						'ed_media_text'           => __( 'Insert', 'uix-usercenter' ),				
						'ed_image'                => __( 'Insert Image', 'uix-usercenter' ),
						'ed_sourcecode_title'     => __( 'Source Code', 'uix-usercenter' ),
						'ed_unlink_title'         => __( 'Remove link', 'uix-usercenter' ),
						'ed_link_title'           => __( 'Insert/Edit link', 'uix-usercenter' ),
						'ed_link_field_url'       => __( 'URL', 'uix-usercenter' ),
						'ed_link_field_text'      => __( 'Link Text', 'uix-usercenter' ),
						'ed_link_field_win'       => __( 'Open link in a new tab', 'uix-usercenter' ),
						'ed_hcode_title'          => __( 'Syntax Highlight Code', 'uix-usercenter' ),
						'ed_hcode_field_label'    => __( 'Language', 'uix-usercenter' ),
                        'select_empty_text'       => __( '-', 'uix-usercenter' ),
                        'delete_confirm_text'     => __( 'Are you sure you want to delete?', 'uix-usercenter' ),
		...
		*/
		public static function add() {
			...
			$all_args = apply_filters( 'uix_usercenter_custom_metaboxes_vars', self::$all_config );
			...
			foreach ( $all_args as $args ) {
				...
				foreach ( $args as $v ) {
					...
					$title     = ( isset( $v[ 'config' ][ 'title' ] ) ) ? esc_html( $v[ 'config' ][ 'title' ] ) : esc_html__( 'Group Title', 'uix-usercenter' );
		...
		*/
		public static function register_meta_boxes( $post, $metabox ) {
			...
			?>
			<!-- Begin Fields -->
			<div class="uix-usercenter-cmb__wrapper">
				<table class="form-table uix-usercenter-cmb">
					...
					foreach ( $fields as $v ) {
						...
						if ( ( isset( $v[ 'id' ] ) && !empty( $v[ 'id' ] ) ) && ( isset( $v[ 'type' ] ) && !empty( $v[ 'type' ] ) ) ) {
							...
							$type          = $v[ 'type' ];
							$id            = esc_attr( $v[ 'id' ] );
							$title         = ( isset( $v[ 'title' ] ) ) ? $v[ 'title' ] : esc_html__( 'Field Title', 'uix-usercenter' );
							...
							//------
							if ( $type == 'text' ) {
								UixUserCenterCmbFormType_Text::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	

							//------
							if ( $type == 'textarea' ) {
								UixUserCenterCmbFormType_Textarea::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	

							//------
							if ( $type == 'url' ) {
								UixUserCenterCmbFormType_Url::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	

							//------
							if ( $type == 'number' ) {
								UixUserCenterCmbFormType_Number::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}				

							//------
							if ( $type == 'radio' ) {
								UixUserCenterCmbFormType_Radio::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}			

							//------
							if ( $type == 'image' ) {
								UixUserCenterCmbFormType_Image::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}						

							//------
							if ( $type == 'color' ) {
								UixUserCenterCmbFormType_Color::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}				

							//------
							if ( $type == 'checkbox' ) {
								UixUserCenterCmbFormType_Checkbox::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}		

							//------
							if ( $type == 'select' ) {
								UixUserCenterCmbFormType_Select::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}					
							
							//------
							if ( $type == 'editor' ) {
								UixUserCenterCmbFormType_Editor::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}					
							
							//------
							if ( $type == 'date' ) {
								UixUserCenterCmbFormType_Date::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}				

							//------
							if ( $type == 'price' ) {
								UixUserCenterCmbFormType_Price::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}		
							
							//------
							if ( $type == 'multi-checkbox' ) {
								UixUserCenterCmbFormType_MultiCheckbox::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	
							
							//------
							if ( $type == 'custom-attrs' ) {
								UixUserCenterCmbFormType_CustomAttrs::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	
							
							
							//------
							if ( $type == 'multi-content' ) {
								UixUserCenterCmbFormType_MultiContent::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	
                            
                            
							//------
							if ( $type == 'multi-portfolio' ) {
								UixUserCenterCmbFormType_MultiPortfolio::add( $id, $title, $desc, $default, $options, $placeholder, $desc_primary, true );
							}	                      
...
Uix_UserCenter_Custom_Metaboxes::admin_ready();	
